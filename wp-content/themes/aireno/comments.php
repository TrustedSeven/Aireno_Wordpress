...
$aireno_comment_count = get_comments_number();
?>
...
<div id="comments" class="comments-area default-max-width <?php echo get_option( 'show_avatars' ) ? 'show-avatars' : ''; ?>">
	...
	if ( have_comments() ) :
		...
		<h2 class="comments-title">
			<?php if ( '1' === $aireno_comment_count ) : ?>
				<?php esc_html_e( '1 comment', TEXT_DOMAIN ); ?>
			<?php else : ?>
				<?php
				printf(
					/* translators: %s: Comment count number. */
					esc_html( _nx( '%s comment', '%s comments', $aireno_comment_count, 'Comments title', TEXT_DOMAIN ) ),
					esc_html( number_format_i18n( $aireno_comment_count ) )
				);
				?>
			<?php endif; ?>
		</h2><!-- .comments-title -->
		...
		the_comments_pagination(
			array(
				...
				'prev_text'          => sprintf(
					...
					esc_html__( 'Prev', TEXT_DOMAIN )
				),
				'next_text'          => sprintf(
					'<span class="nav-next-text">%s</span> %s',
					esc_html__( 'Next', TEXT_DOMAIN ),
